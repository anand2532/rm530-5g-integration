...
def create_status_table(status: HealthStatus) -> Union[Table, None]:  # type: ignore[return-value]
    ...
    health_text = (
        ...
        else "[bold red]✗ Unhealthy[/bold red]"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Monitor RM530 5G connection health")
    parser.add_argument(
        "--interface", "-i", default="usb0", help="Network interface name (default: usb0)"
    )
    parser.add_argument(
        "--interval", type=int, default=60, help="Health check interval in seconds (default: 60)"
    )
    parser.add_argument(
        "--threshold", type=int, default=3, help="Failure threshold before alerting (default: 3)"
    )
    parser.add_argument("--once", action="store_true", help="Run health check once and exit")
    parser.add_argument(
        "--live", action="store_true", help="Show live updating dashboard (requires rich)"
    )
            ...
            def on_status_change(status: HealthStatus):
                ...
                if status.is_healthy:
                    ...
                    if RICH_AVAILABLE and console is not None:
                        ...
                        print(
                            f"[{timestamp}] ✗ Connection unhealthy (Failures: {status.consecutive_failures})"
                        )
                        if status.issues:
                            for issue in status.issues:
                                print(f"  • {issue}")
                ...
                if status.consecutive_failures >= args.threshold:
                    if RICH_AVAILABLE and console is not None:
                        ...
                        print("=" * 60)
                        print(
                            f"ALERT: Connection has failed {status.consecutive_failures} times consecutively!"
                        )
                        print(f"Issues: {', '.join(status.issues)}")
                        print("=" * 60)
